...
short CalculationDays(sDate DateFrom , sDate DateTo)
{
	short Days = 0;
	while (DateOneBeforeDateTwo(DateFrom , DateTo))
	{
		if (IsBeusinessDays(DateFrom))
		{
			Days++;
		}
...
int main() 
{
	...
	cout << "\nVacation Ends : \n";
	sDate VacationEnd = ReadDate();

	cout << "\nVacation From : " << ShortNameDays(VacationStart) << " , " << VacationStart.Day << "/" << VacationStart.Month << "/" << VacationStart.Year;
	cout << "\nVacation From : " << ShortNameDays(VacationEnd) << " , " << VacationEnd.Day << "/" << VacationEnd.Month << "/" << VacationEnd.Year;
	cout << "\nActucal Vacation Days is : " << CalculationDays(VacationStart, VacationEnd);
